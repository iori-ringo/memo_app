...
import { NotePage, CanvasObject, Connection, Stroke, SectionType } from "@/types/note";
import { DEFAULT_OBJECT_SIZE, DEFAULT_NEW_OBJECT_OFFSET, SECTION_TYPES } from "@/lib/constants";
...
export function useCanvasOperations(
    ...
    const handleAddBlock = useCallback((eOrX: React.MouseEvent | number, valY?: number) => {
        let x: number;
        let y: number;
    ...
    const handleUpdateObject = useCallback((objectId: string, updates: Partial<CanvasObject>) => {
